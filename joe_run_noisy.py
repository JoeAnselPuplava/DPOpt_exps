...
def run_queries(db_info, q_dict, queue):
    '''
...
def main():
    ...
    try:
        ...
        for i in range(10):
            ...
            process = multiprocessing.Process(target=run_queries, args=(db_info, q_dict,queue,))
            process.start()

            # wait 900s for the thread running the query in case it is taking too long
            timeout_seconds = 900
            print(f"[Parent {os.getpid()}] Waiting {timeout_seconds} seconds for child...")
            process.join(timeout_seconds)
            
            
            if process.is_alive():
                print(f"[Parent] Timeout! Terminating child process {process.pid}")
                process.terminate()
                process.join()
                raise Exception(f"Child process exceeded timeout of {timeout_seconds} seconds")
            
            exec_times = queue.get() # try to get the output after the check for timeout
